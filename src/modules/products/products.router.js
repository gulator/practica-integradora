    ...
    this.post("/", ["PREMIUM", "ADMIN"], async (req, res) => {
      try {
        ...
        if (value === false) {
          res
            .status(400)
            .send({ status: 400, error: "Campos incompletos o incorrectos" });
        } else if (value === "code repeated") {
          res
            .status(400)
            .send({
              status: 400,
              error: "El campo code ya se encuentra cargado para otro producto",
            });
        } else {
          socketServer.emit("update", await productController.getAllproducts());
          req.logger.info(
            `Product ${value._id} created by ${
              value.owner
    ...
    this.put("/:pid", ["PREMIUM", "ADMIN"], async (req, res) => {
      try {
        ...
        if (!productEdit) {
          ...
          res
            .status(400)
            .send({ error: "el campo ID no puede ser modificado" });
        } else if (productEdit === 404) {
          res
            .status(404)
            .send({
              error: "El id indicado no corresponde a un producto valido",
            });
        } else {
